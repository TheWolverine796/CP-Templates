...
bool compare(pair<pair<int, int>, int> p1, pair<pair<int, int>, int> p2) {
  int b1 = p1.first.first / block_size;
  int b2 = p2.first.first / block_size;
  if (b1 == b2)
    return b1 % 2 == 0 ? (p1.first.second < p2.first.second)
...
int main() {
  ...
  for (auto i : ans) {
    ...
    int right = p1.first.second - 1;
    int left = p1.first.first - 1;
    while (start < left) {
      int x = arr[start];
      freq[x]--;
      if (freq[x] == 0)
